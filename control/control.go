...
func (c Control) isAppRegistered() bool {
	resp, err := http.Get("http://service-ricplt-appmgr-http.ricplt:8080/ric/v1/xapps")
	if err != nil {
...
func (c Control) xAppStartCB(d interface{}) {
	...
	for {
		...
		if !c.isAppRegistered() {
			xapp.Logger.Debug("Application='%s' is not registered yet, waiting ...", viper.GetString("name"))
			continue
		}
